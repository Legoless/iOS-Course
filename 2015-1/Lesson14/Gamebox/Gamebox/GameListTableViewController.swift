...
class GameListTableViewController: UITableViewController {
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        if cell.gestureRecognizers == nil || cell.gestureRecognizers?.count == 0 {
            let recognizer = UILongPressGestureRecognizer(target: self, action: "deleteGame:")
    ...
    func deleteGame(recognizer: UIGestureRecognizer) {
        
        if let cell = recognizer.view as? UITableViewCell {
            let indexPath = tableView.indexPathForCell(cell)!
            let game = games[dates[indexPath.section]]![indexPath.row]
            
            let alertController = UIAlertController(title: game.name, message: "Select your action:", preferredStyle: .ActionSheet)
            alertController.addAction(UIAlertAction(title: "Delete", style: .Destructive, handler: { action in
                
                //
                // Delete logic, remove game from the model
                //
                
                if let games = GameManager.shared.games {
                    let index = games.indexOf({ $0 === game})
                    
                    var mutableGames = games
                    mutableGames.removeAtIndex(index!)
                    
                    GameManager.shared.games = mutableGames
                    GameManager.shared.save()
